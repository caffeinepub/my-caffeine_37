...
export default function PaymentHistoryView({ onBack }: PaymentHistoryViewProps) {
  ...
  const loadEntries = () => {
    setIsLoading(true);
    try {
      const histories = safeGetItem<Histories>('histories', { loan: [] });
      const loanArray = Array.isArray(histories?.loan) ? histories.loan : [];
      
      // Filter entries for current user
      const userEntries = loanArray.filter((entry) =>
      ...
      <Card>
        <CardHeader className="bg-gradient-to-r from-rose-500 to-pink-500">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-lg text-white section-title-accent">পেমেন্ট ইতিহাস</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="pt-6">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {entries.map((entry) => (
                <Card key={entry.id} className="border-rose-200">
